...
assembly.all <- read_csv(here::here("model/AssemblyDistricts_with_ModelledVote.csv"))
seats.by.scenario <- read_csv(here::here("model/AssemblySeats_by_Scenario.csv"))
allocation.factors <- read_csv(here::here("WI_2022_ReportingUnit_to_District_Allocation_Factors.csv")) %>%
  group_by(plan) %>%
  mutate(allocated_votes = total_votes * prop_of_rep_unit) %>%
  summarise(no_uncertainty = sum(total_votes[prop_of_rep_unit == 1])/sum(total_votes)*100,
            uncertain_weighted_mean = weighted.mean(prop_of_rep_unit[prop_of_rep_unit < 1],
                                                    w = allocated_votes[prop_of_rep_unit < 1])*100,
            overall_weighted_mean = weighted.mean(prop_of_rep_unit, w = allocated_votes)*100) %>%
...
```{r}
seats.by.scenario %>%
  group_by(plan, baseline) %>%
  summarise(`seats won by Dems` = dem[swing == 0],
            `swing needed for GOP supermajority` = max(swing[rep == 66]),
            `swing needed for Dem majority` = min(swing[dem == 50]),
            .groups = "drop") %>%
  pivot_longer(cols = -c(plan, baseline)) %>%
  pivot_wider(names_from = baseline, values_from = value) %>%
  mutate(plan = case_when(
    plan == "2012" ~ "Districts used from 2012-20",
    plan == "2022" ~ "Districts established in 2022",
    TRUE ~ plan
  ),
  plan = factor(plan, levels = c("Districts established in 2022",
  ...
  gt(rowname_col = "name") %>%
  cols_label(
    final_wsa = "modelled vote",
    GOV = "2022 Gov.",
    USS = "2022 Senator"
  ) %>%
  tab_spanner(columns = c("final_wsa", "GOV", "USS"),
              label = "Distribution of votes based on") %>%
  tab_style(
    style = cell_text(align = "right", indent = px(10), style = "italic"),
    locations = cells_stub()
  ) %>%
  tab_stubhead("Assembly District Map") %>%
  tab_header(title = "Partisan implications of selected real and hypothetical WI Assembly Maps",
             subtitle = "Based on the results of the November 2022 general election") %>%
  tab_style(style = cell_text(weight = "bold"),
            locations = list(cells_row_groups(), cells_column_spanners(),
                             cells_stubhead())) %>%
  tab_style(style = cell_text(weight = "bold", size = "x-large"),
            locations = list(cells_title("title"))) %>%
  tab_style(style = cell_text(weight = "bold", size = "large"),
            locations = list(cells_title("subtitle"))) %>%
  fmt_number(columns = 3:5, rows = str_detect(name, "seats"),
             decimals = 0, pattern = "{x} seats") %>%
  fmt_number(columns = 3:5, rows = str_detect(name, "swing"),
             decimals = 1, pattern = "{x} pts") %>%
  tab_options(table.width = 700) %>%
  tab_footnote("Based on a linear model incorporating the results of the elections for governor, U.S. senate, attorney general, and state treasurer.",
               locations = cells_column_labels("final_wsa")) %>%
  tab_footnote("Tony Evers (D) defeated Tim Michels (R) by 3.4 points. This column shows the Assembly seats where Evers carried a majority of the vote.",
               locations = cells_column_labels("GOV")) %>%
  tab_footnote("Ron Johnson (R) defeated Mandela Barnes (D) by 1.0 points. This column shows the Assembly seats where Johnson carried a majority of the vote.",
               locations = cells_column_labels("USS")) %>%
  tab_footnote("Since these are the actual districts used, I use the real Assembly results for the 73 races which drew candidates from both parties, and I substitute the modelled vote for the 26 uncontested seats.",
               locations = cells_body(columns = "final_wsa", rows = 1))
```
...
#| fig-width: 12
assembly.all %>%
  filter(contested_2022 == "contested") %>%
  select(plan, district, WSA, predict_avg, predict_v1, predict_v2) %>%
  pivot_longer(cols = starts_with("predict"), names_to = "method", values_to = "predicted") %>%
  mutate(residuals = WSA - predicted,
         method = case_when(
           method == "predict_avg" ~ "simple average of 4 races",
           method == "predict_v1" ~ "model, v1",
           method == "predict_v2" ~ "model, v2"
         ),
         method = factor(method, levels = c("simple average of 4 races",
                                            "model, v1",
                                            "model, v2"))) %>%
  ggplot(aes(WSA, residuals)) +
  geom_point(alpha = 0.75) +
  ggrepel::geom_text_repel(data = function(x){filter(x, abs(residuals) > 6)},
                           aes(label = district), min.segment.length = 0.1) +
  labs(x = "actual result (Democratic vote margin)",
       y = "residuals (real - predicted)",
       title = "Comparison of actual Assembly vote with residuals from 3 prediction methods") +
